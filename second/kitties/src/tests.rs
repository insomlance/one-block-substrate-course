...
fn new_kitty(account_id: u64) -> DispatchResultWithPostInfo {
...
fn transfer_fail_not_owner() {
	new_test_ext().execute_with(|| {
		let _ = new_kitty(1);
		assert_noop!(Kitties::transfer(Origin::signed(2), 1, 1), Error::<Test>::NotCorrectOwner);
	});
}
...
fn breed_fail_with_same_kitty_id() {
	new_test_ext().execute_with(|| {
		assert_noop!(Kitties::breed(Origin::signed(1), 1, 1), Error::<Test>::BreedFromSameParent);
	});
}
...
fn breed_fail_with_invalid_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Kitties::breed(Origin::signed(1), 1, 2), Error::<Test>::NotCorrectOwner);
	});
}
